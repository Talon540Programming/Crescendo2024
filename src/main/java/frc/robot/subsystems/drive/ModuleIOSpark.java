...
// import edu.wpi.first.math.controller.SimpleMotorFeedforward; //6328 does this in Module
// import edu.wpi.first.math.filter.Debouncer; //6328 does this in Module
...
public class ModuleIOSpark implements ModuleIO {
  ...
  public ModuleIOSpark(int index) {
    switch (Constants.getRobot()) {
      case OUTREACHBOT -> config = moduleConfigs[index];
      default ->
    ...
    turnRelativeEncoder.setPosition(getOffsetAbsoluteAngle().getRadians());

    drivePositionQueue = OdometryManager.getInstance().registerSignal(driveEncoder::getPosition);
    turnPositionQueue =
        OdometryManager.getInstance().registerSignal(turnRelativeEncoder::getPosition);
  }
